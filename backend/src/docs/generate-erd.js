...
function parseAssociations() {
  ...
  const hasManyRegex =
    /(\w+)\.hasMany\((\w+),\s*\{[^}]*foreignKey:\s*["'](\w+)["'][^}]*as:\s*["'](\w+)["']/g;
  let match;
  ...
  // Parse belongsTo
  const belongsToRegex =
    /(\w+)\.belongsTo\((\w+),\s*\{[^}]*foreignKey:\s*["'](\w+)["'][^}]*as:\s*["'](\w+)["']/g;
  ...
  // Parse belongsToMany
  const belongsToManyRegex =
    /(\w+)\.belongsToMany\((\w+),\s*\{[^}]*through:\s*(\w+),[^}]*foreignKey:\s*["'](\w+)["'][^}]*otherKey:\s*["'](\w+)["'][^}]*as:\s*["'](\w+)["']/g;
...
function generateAll() {
  ...
  for (const file of MODEL_FILES) {
    ...
    if (fs.existsSync(filePath)) {
      ...
      models.push(model);
      console.log(
        `✅ Parsed ${model.modelName} (${model.tableName}) - ${model.columns.length} columns`,
      );
    }
  }
  ...
  console.log(
    "💡 Or render with: npx @mermaid-js/mermaid-cli -i docs/database/erd.mmd -o docs/database/",
  );
