      ...
      schemas: {
        ...
        Balance: {
          ...
          properties: {
            ...
            balance: { type: "number", format: "float", example: -25.0 },
            currency: { type: "string", example: "USD" },
          },
        },
        Error: {
...
function generateOpenApiSpec() {
  ...
  for (const outputDir of outputDirs) {
    ...
    fs.writeFileSync(path.join(outputDir, "openapi.json"), JSON.stringify(swaggerSpec, null, 2));

    fs.writeFileSync(path.join(outputDir, "openapi.yaml"), require("js-yaml").dump(swaggerSpec));
